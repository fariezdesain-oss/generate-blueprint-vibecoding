...
import { Plus, X, FileText, Wand2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <button
        ...
        className="group mb-4 flex items-center gap-2 rounded-xl border border-dashed border-subtle px-3 py-3 text-sm font-semibold text-tertiary transition-all duration-200 hover:border-gemini-blue/30 hover:bg-tertiary hover:text-primary"
      >
        <Plus size={16} />
        <span>New Chat</span>
      </button>
      ...
      {loading && sessions.length === 0 ? (
        <div className="flex justify-center py-4">
          <Wand2 size={24} className="animate-wand-swing text-tertiary" />
        </div>
      ) : sessions.length === 0 ? (
        <p className="px-3 text-xs text-tertiary">No sessions yet</p>
      ) : (
            ...
            return (
              <div
                ...
                className={`group flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2.5 text-sm transition-all duration-200 ${
                  isActive
                    ? 'bg-tertiary text-primary font-bold border-2 border-subtle'
                    : 'text-tertiary font-semibold hover:bg-tertiary hover:text-secondary'
                }`}
              >
                <span className="flex-1 truncate">{s.title}</span>
                {s.has_generated && (
                  <span className="shrink-0 rounded-full bg-gemini-blue px-2.5 py-0.5 text-[10px] font-bold text-white">
                    DOCS
                  </span>
                )}
                {s.has_n8n && (
                  <span className="shrink-0 rounded-full bg-gemini-blue px-2.5 py-0.5 text-[10px] font-bold text-emerald-400">
                    N8N
                  </span>
                )}
